...
function parseFullPath(fullPath) {
    ...
    if (qparamsIndex >= 0) {
        const qparams = path.slice(qparamsIndex + 1);
        ...
        if (!qparams) {
            qparams.split('&').forEach(qparam => {
                ...
                let qvalue = '';
                if (qparamIndex >= 0) {
                    qname = decodeURIComponent(qparam.slice(0, queryIndex));
                    qvalue = decodeURIComponent(qparam.slice(queryIndex + 1));
                }

                const prevQvalue = query[qname];
                if (!prevQvalue) {
                    query[qname] = qvalue;
                } else if (Array.isArray(prevQvalue)) {
                    prevQvalue.append(qvalue);
                } else {
                    query[qname] = [ prevQvalue, qvalue ];
                }
            });
        }
    }
