...
class HistoryState {
    ...
    findBackPosition(location) {
        ...
        if (typeof location === 'string') {
            ...
            const url = new URL(location.fullPath);
            if (!location.path && url.pathname) {
                location.path = url.pathname;
            }
            if (!location.query && url.search) {
                const params = new URLSearchParams(url.search);
                location.query = {};
                for (let key in params.keys()) {
                    const values = params.getAll(key);
                    if (values.length > 1) {
                        location.query[key] = values;
                    } else if (values.length == 1) {
                        location.query[key] = values[0];
                    }
                }
            }
        }
    ...
    _init(router) {
        ...
        router.afterEach((to, from) => {
            ...
            if (reloadable && !(to.query && to.query['_p'])) {
    ...
    _validate() {
        ...
        if (this._items[this._page]) {
            ...
            if (backupRoute != null && !equal(backupRoute, filterRoute(this._route))) {
                this._action = 'invalid';
                this._page = 0;
                this._items = Array(1);
